...
/**
 * Translates a subset of Elasticsearch Query DSL JSON into Lucene Query objects.
 * Supports: match_all, term, terms, bool, range, wildcard, prefix, exists.
 */
public class SimpleQueryTranslator {

    public static Query translate(String queryJson) throws IOException {
        try (XContentParser parser = XContentType.JSON.xContent().createParser(XContentParserConfiguration.EMPTY, queryJson)) {
    ...
    private static Query parseBool(XContentParser parser) throws IOException {
        ...
        while (parser.nextToken() != XContentParser.Token.END_OBJECT) {
            ...
            parser.nextToken();
            BooleanClause.Occur occur = switch (clauseType) {
    ...
    private static Query parseRange(XContentParser parser) throws IOException {
        ...
        if (isNumeric) {
            ...
            return new TermRangeQuery(field, lowerTerm != null ? new org.apache.lucene.util.BytesRef(lowerTerm) : null,
                upperTerm != null ? new org.apache.lucene.util.BytesRef(upperTerm) : null, includeLower, includeUpper);
        }
    }
