...
public class ListSnapshotsCli extends Command {
    ...
    indexOption =
        parser
            .acceptsAll(
                ...
                "Filter: only show snapshots containing this index or alias")
            .withRequiredArg();
  ...
  protected void execute(Terminal terminal, OptionSet options, ProcessInfo processInfo)
    ...
    try (S3ClientFactory.S3Access s3Access = s3Options.connect(options)) {
      ...
      if (indexFilter != null) {
        ...
        List<SnapshotMetadataLoader.SnapshotSummary> snapshots =
            loader.listSnapshotsForIndex(indexFilter);
        ...
        if (options.has(jsonOption)) {
          ...
          terminal.println(
              String.format(
                  "%-40s  %-10s  %-20s  %-20s  %s",
                  "SNAPSHOT", "STATE", "START", "END", "INDICES"));
          terminal.println("-".repeat(130));
          for (var snap : snapshots) {
            String start =
                snap.startTimeMillis() > 0
                    ? FMT.format(Instant.ofEpochMilli(snap.startTimeMillis()))
                    : "n/a";
            String end =
                snap.endTimeMillis() > 0
                    ? FMT.format(Instant.ofEpochMilli(snap.endTimeMillis()))
                    : "n/a";
            String indices =
                snap.indices().size() <= 5
  ...
  private static String renderJson(List<SnapshotMetadataLoader.SnapshotSummary> snapshots)
      throws Exception {
