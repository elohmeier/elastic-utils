...
public class SnapshotQueryCli extends Command {
  ...
  public SnapshotQueryCli() {
    ...
    snapshotOption =
        parser
            .acceptsAll(Arrays.asList("s", "snapshot"), "Snapshot name")
            .withRequiredArg()
            .required();
    indexOption =
        parser.acceptsAll(Arrays.asList("i", "index"), "Index name").withRequiredArg().required();
    queryFileOption =
        parser.accepts("query-file", "Path to JSON file containing Query DSL").withRequiredArg();
    queryOption =
        parser.acceptsAll(Arrays.asList("q", "query"), "Inline Query DSL JSON").withRequiredArg();
    sizeOption =
        parser
            .accepts("size", "Maximum number of results")
            .withRequiredArg()
  ...
  protected void execute(Terminal terminal, OptionSet options, ProcessInfo processInfo)
    ...
    try (S3ClientFactory.S3Access s3Access = s3Options.connect(options)) {
      ...
      SnapshotMetadataLoader.ResolvedIndex resolved =
          metadataLoader.resolve(snapshotName, indexName);
          ...
          if (sourceField != null) {
            builder.rawField(
                ...
                new java.io.ByteArrayInputStream(
                    sourceField.bytes, sourceField.offset, sourceField.length),
                XContentType.JSON);
          }
          builder.endObject();
        }
