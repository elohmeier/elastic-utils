...
public class SnapshotExportCli extends Command {
  ...
  public SnapshotExportCli() {
    ...
    snapshotOption =
        parser
            .acceptsAll(Arrays.asList("s", "snapshot"), "Snapshot name (auto-detected if omitted)")
            .withRequiredArg();
    indexOption =
        parser
    ...
    outputOption =
        parser
            .acceptsAll(Arrays.asList("o", "output"), "Output file path (default: stdout)")
            .withRequiredArg();
    compressionOption =
        parser
    ...
    try (S3ClientFactory.S3Access s3Access = s3Options.connect(options, profilingRecorder);
      ...
      if (snapshotName == null) {
        ...
        terminal.errorPrintln(
            "No snapshot specified, searching for snapshots containing ["
                + indexNameOrAlias
                + "]...");
        ...
        if (candidates.isEmpty()) {
          throw new UserException(
              ExitCodes.CONFIG,
              "No snapshots found containing index/alias [" + indexNameOrAlias + "]");
        }
        ...
        if (candidates.size() > 1) {
          ...
          terminal.errorPrintln(
              "  (other candidates: "
                  + String.join(", ", candidates.subList(1, Math.min(5, candidates.size())))
                  + (candidates.size() > 5 ? " ..." : "")
                  + ")");
        }
      }
      ...
      List<SnapshotMetadataLoader.ResolvedIndex> resolvedIndices =
          metadataLoader.resolveIndices(snapshotName, indexNameOrAlias);
      ...
      for (SnapshotMetadataLoader.ResolvedIndex resolved : resolvedIndices) {
        ...
        profilingRecorder.startTarget(
            snapshotName, resolved.indexId().getName(), current - 1, resolvedIndices.size());
        totalExported +=
      ...
      progressReporter.clearLine();
      terminal.errorPrintln(
          "Export complete: " + totalExported + " documents in " + (tookMs / 1000) + "s");
    }
    ...
    try (XContentParser parser =
      ...
      while (parser.nextToken() != XContentParser.Token.END_OBJECT) {
        ...
        if ("query".equals(field)
            || "sort".equals(field)
            || "_source".equals(field)
            || "track_total_hits".equals(field)) {
          return true;
        }
        parser.nextToken();
        parser.skipChildren();
