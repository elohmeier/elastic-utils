...
public class SearchBodyParser {
  ...
  public static SearchBodyParser parse(String json, String fromDate, String toDate)
      throws IOException {
    ...
    if (result.sort == null) {
      result.sort =
          new Sort(
              new SortedNumericSortField("@timestamp", SortField.Type.LONG), SortField.FIELD_DOC);
    }
  ...
  /**
   * Parse just the query portion (for --query / --query-file that only contain a query, not full
   * body).
   */
  public static SearchBodyParser fromQueryOnly(String queryJson, String fromDate, String toDate)
      throws IOException {
    SearchBodyParser result = new SearchBodyParser();
    result.sourceFields = null;
    ...
    while (parser.nextToken() != XContentParser.Token.END_ARRAY) {
      if (parser.currentToken() == XContentParser.Token.START_OBJECT) {
        ...
        if ("@timestamp".equals(fieldName)) {
          ...
          sortFields.add(
              reverse ? new SortField(null, SortField.Type.DOC, true) : SortField.FIELD_DOC);
        } else {
