...
/**
 * Read-only Lucene Directory backed by S3 via a BlobContainer. Maps Lucene file names to blob
 * names/parts using BlobStoreIndexShardSnapshot metadata.
 */
public class SnapshotQueryDirectory extends BaseDirectory {
  ...
  public SnapshotQueryDirectory(
      BlobContainer blobContainer, BlobStoreIndexShardSnapshot shardSnapshot) {
  ...
  public IndexInput openInput(String name, IOContext context) throws IOException {
    ...
    if (fileInfo.metadata().hash() != null
        && fileInfo.metadata().hash().length > 0
        && fileInfo.metadata().hashEqualsContents()) {
  ...
  static class S3IndexInput extends BufferedIndexInput {
    ...
    protected void readInternal(ByteBuffer b) throws IOException {
      ...
      while (remaining > 0) {
        ...
        int toFetch =
            Math.toIntExact(
                Math.min(length - (pos - offset), Math.max(remaining, readAheadBytes())));
    ...
    protected void seekInternal(long pos) {
      ...
      if (pos < 0 || pos > length) {
        throw new IllegalArgumentException(
            "Seek position " + pos + " out of range [0, " + length + "]");
      }
    }
    ...
    private boolean readaheadContains(long pos) {
      return readaheadCache != null
          && pos >= readaheadStart
          && pos < readaheadStart + readaheadLength;
    }
      ...
      while (written < len) {
        ...
        if (part >= fileInfo.numberOfParts()) {
          throw new EOFException(
              "Read past end of file: position=" + pos + " length=" + fileInfo.length());
        }
        ...
        try (InputStream is =
            blobContainer.readBlob(OperationPurpose.SNAPSHOT_DATA, blobName, posInPart, toRead)) {
          int partRead = is.readNBytes(result, written, toRead);
          written += partRead;
          pos += partRead;
          if (partRead < toRead) {
            throw new EOFException(
                "Short read from S3: expected " + toRead + " bytes but got " + partRead);
          }
        }
      }

      if (profilingRecorder != null && indexName != null) {
        profilingRecorder.recordLuceneFileRead(
            indexName,
            shardId,
            fileInfo.physicalName(),
            written,
            System.nanoTime() - readStartNanos);
      }
      return result;
    }
  }
  ...
  /**
   * Simple IndexInput backed by a byte array, for small files whose content is embedded in
   * metadata.
   */
