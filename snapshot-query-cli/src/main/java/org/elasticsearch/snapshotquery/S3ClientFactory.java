...
public class S3ClientFactory {
  ...
  public static S3Access create(
    ...
    if (trustAllCerts) {
      httpBuilder.tlsTrustManagersProvider(
          () ->
  ...
  /**
   * Minimal read-only BlobContainer backed by S3Client. Only implements the methods needed for
   * snapshot metadata reading and Lucene index access.
   */
  static class SimpleS3BlobContainer implements BlobContainer {
    ...
    public InputStream readBlob(OperationPurpose purpose, String blobName) throws IOException {
      ...
      try {
        GetObjectRequest request =
            GetObjectRequest.builder().bucket(bucket).key(blobKey(blobName)).build();
        ...
        throw new NoSuchFileException(
            "Blob [" + blobKey(blobName) + "] not found in bucket [" + bucket + "]");
      }
    }

    @Override
    public InputStream readBlob(
        OperationPurpose purpose, String blobName, long position, long length) throws IOException {
      long startNanos = System.nanoTime();
      try {
        String range = "bytes=" + position + "-" + (position + length - 1);
        GetObjectRequest request =
            GetObjectRequest.builder().bucket(bucket).key(blobKey(blobName)).range(range).build();
        InputStream inputStream = client.getObject(request);
        if (profilingRecorder == null) {
          return inputStream;
        }
    ...
    public Map<String, BlobMetadata> listBlobsByPrefix(
        OperationPurpose purpose, @Nullable String blobNamePrefix) throws IOException {
    ...
    @Override
    public org.elasticsearch.common.blobstore.DeleteResult delete(OperationPurpose purpose)
        throws IOException {
      throw new UnsupportedOperationException("Read-only blob container");
    }
    ...
    public void deleteBlobsIgnoringIfNotExists(
        OperationPurpose purpose, java.util.Iterator<String> blobNames) throws IOException {
      throw new UnsupportedOperationException("Read-only blob container");
    }
  ...
  private static final class ProfilingInputStream extends InputStream {
    ...
    public void close() throws IOException {
      ...
      try {
        ...
        profilingRecorder.recordS3Read(
            ranged, Math.max(requestedBytes, bytesRead), bytesRead, System.nanoTime() - startNanos);
      }
    }
  }
}
