 ...
 *   S3_BUCKET, S3_BASE_PATH, S3_REGION, S3_ENDPOINT,
 *   S3_ACCESS_KEY (or AWS_ACCESS_KEY_ID),
 *   S3_SECRET_KEY (or AWS_SECRET_ACCESS_KEY),
 *   S3_TRUST_ALL_CERTS, S3_RESOLVE
 */
public class S3Options {
    ...
    public S3Options(OptionParser parser) {
        bucket = parser.acceptsAll(Arrays.asList("b", "bucket"), "S3 bucket name (env: S3_BUCKET)").withRequiredArg();
        basePath = parser.accepts("base-path", "Base path in S3 bucket (env: S3_BASE_PATH)").withRequiredArg();
        region = parser.accepts("region", "AWS region (env: S3_REGION)").withRequiredArg();
        endpoint = parser.accepts("endpoint", "Custom S3 endpoint URL (env: S3_ENDPOINT)").withRequiredArg();
        accessKey = parser.accepts("access-key", "AWS access key (env: S3_ACCESS_KEY or AWS_ACCESS_KEY_ID)").withRequiredArg();
        secretKey = parser.accepts("secret-key", "AWS secret key (env: S3_SECRET_KEY or AWS_SECRET_ACCESS_KEY)").withRequiredArg();
        trustAllCerts = parser.accepts("trust-all-certs", "Disable TLS certificate verification (env: S3_TRUST_ALL_CERTS)");
        resolve = parser.accepts("resolve", "Resolve hostname to IP, hostname:ip (env: S3_RESOLVE)").withRequiredArg();
    }
    ...
    public S3ClientFactory.S3Access connect(OptionSet options) {
        ...
        String secretKeyVal = resolve(options, secretKey, "S3_SECRET_KEY", env("AWS_SECRET_ACCESS_KEY"));
    ...
    private static String resolve(OptionSet options, OptionSpec<String> spec, String envVar, String defaultVal) {
        if (options.has(spec)) {
